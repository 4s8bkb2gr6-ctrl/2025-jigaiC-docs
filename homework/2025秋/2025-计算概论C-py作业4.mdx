...
<Tabs>
  ...
  <Tab title="钱丽艳">
    ```python 02：删除子串 lines icon="python"
    def strFindDel(longS, shortS, n):
        if longS.find(shortS) != -1:
            if not n:       # 只删除一次
                return longS.replace(shortS, "", 1)
            else:           # 删除所有找到的
                return longS.replace(shortS, "")
        else:
            return "Fail to delete"

    def main():
        longStr, shortStr, boo = input().split()
        boo = int(boo)
        print(strFindDel(longStr, shortStr, boo))
  ...
  <Tab title="钱丽艳">
    ...
    def getNumbers(s):
        ...
        for i in '+-*/':
            if i in expS:
                op = expS[expS.find(i)]
                x, y = expS.replace(i, ' ').split()
                break
        return x, y, z, op


    string = input()
    x, y, z, op = getNumbers(string)
    if round(eval(x + op + y), 2) == z:
        print("true")
    else:
        print("false")
    ```
    ```python 04：自动判题 Solution 2 lines icon="python"
    # Solution 2
    def getNumbers(expR, ans):
        expR, ans = eval(expR), float(ans)
        return expR, ans

    expS, zS = input().split('=')
    expResult, z = getNumbers(expS, zS)
    if round(expResult, 2) == round(z, 2):
        print("true")
    else:
        print("false")
    ```
...
<Tabs>
  ...
  <Tab title="钱丽艳">
    ```python 05：凯撒密码 lines icon="python"
    def getTranslatedMsg(mode, key, msg):
        if mode.lower() == 'd' or mode.lower() == 'decrypt':   
            key = -key  # 解密相当于加密的逆运算，如果是解密，密钥取反
        elif mode.lower() != 'e' and mode.lower() != 'encrypt':
            return 'Wrong Mode'
        translatedMsg = ''
        for symbol in msg:
            ordS = ord(symbol)
            if symbol.isupper():        # 大写字母
                p = ord('A') + (ordS - ord('A') + key) % 26
            elif symbol.islower():      # 小写字母
                p = ord('a') + (ordS - ord('a') + key) % 26
            elif symbol.isnumeric():     # 数字
                p = ord('0') + (ordS - ord('0') + key) % 10
            else:
                p = ordS
            translatedMsg += chr(p)
    ...
    def main():
        ...
        key = int(input())
        msg = input()
        print(getTranslatedMsg(mode, key, msg))
